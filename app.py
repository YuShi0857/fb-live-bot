        ...
        with st.spinner("AI 靈感湧現中，請稍候... (若數量較多請耐心等待)"):
            try:
                ...
                if length_req == "長短隨機交錯 (5-50字，最自然)":
                    length_prompt = "【極致長短交錯】(極重要！) 嚴禁每則留言長度都差不多！請強制分配：大約 30% 必須是極短句 (10字以內，例如「買了」、「等超久」、「好用」)，40% 是中等長度 (15-30字)，30% 是長句心得 (30-50字)。"
                else:
                    length_prompt = f"字數長度：{length_req}。"
                ...
                4. 禁忌命令與代稱 (極重要)：絕對禁止生成只有「+1」、「+2」或單純「商品名+1」的無意義留言！同時，盡量搭配使用「這個」、「這罐」、「這組」、「這件」來代稱商品，讓語氣更自然。
...
if not st.session_state.history:
    ...
    for record in st.session_state.history:
        with st.sidebar.expander(f"📂 {record['product']}"):
            ...
            st.write(record['content'])import streamlit as st
import google.generativeai as genai
from PIL import Image
import uuid
