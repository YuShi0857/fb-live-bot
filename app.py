        ...
        with st.spinner("AI 靈感湧現中，請稍候... (若數量較多請耐心等待)"):
            try:
                ...
                if length_req == "長短隨機交錯 (5-50字，最自然)":
                    length_prompt = "【極致長短交錯】嚴禁每則留言長度都差不多！請強制分配：大約 30% 是簡短完整句子 (10-15 字)，40% 是中等長度 (20-30字)，30% 是長句心得 (30-50字)。"
                else:
                    length_prompt = f"字數長度：{length_req}。注意：必須是語意完整的通順短句。"

                # 表情符號死命令
                if emoji_freq == 0:
                    emoji_prompt = "【格式死命令】絕對禁止出現任何表情符號 (Emoji)！連一個都不准有，必須是 100% 純文字！"
                else:
                    emoji_prompt = f"適度使用表情符號 (活躍度 {emoji_freq}/5)。"
                ...
                【嚴格留言要求】
                1. 總數與格式：必須產生 {num_comments} 則留言，請用條列式輸出。【直接給清單，絕對禁止任何開場白或結語說明】。
                2. 字數要求：{length_prompt}
                3. 語氣限制：貼近台灣 FB 直播買家習慣。絕對禁止出現質疑或比價。所有留言都必須是正向支持。
                4. 禁忌命令與代稱：絕對禁止生成只有「+1」、「好用」等沒頭沒尾的單詞！每一則留言都必須是完整的句子。盡量搭配使用「這個」、「這罐」、「這組」來代稱商品。
                5. 稱呼限制：**絕對禁止使用「小編」**！隨機使用以下稱呼：【{host_instruction}】。大約每 10 則留言出現 1 次稱呼即可。
                6. 留言者人設：分配給【{persona_instruction}】。特別注意：若是「通用」人設，嚴禁提及特定性別、年齡。
                7. 表情符號要求：{emoji_prompt}
                8. 標點符號排版：一半 (50%) 留言維持正常標點符號；**另一半 (50%) 留言請「完全不要使用標點符號」，直接用「半形空格」代替斷句**。
                9. 風格分配：請依照以下比例混搭撰寫：
                   - 渴望型 (求關注/想買/正向驚呼)：{style_desire}份
                   - 划算型 (覺得超值/直接下單/詢問組合優惠)：{style_value}份
                   - 功效型 (期待效果/覺得設計很好)：{style_effect}份
                   - 提問型 (正向詢問使用細節、出貨時間等，絕無質疑)：{style_question}份
                   - 反饋型 (分享過去買過的真實好用經驗)：{style_feedback}份
                   - 互動型 (閒話家常：例如剛下班趕來看、稱讚主播今天穿搭/氣色、問候大家。【絕對禁止】說「畫面清晰」、「畫質真好」這種機器人才會說的假話！)：{style_interaction}份
                """
                
                if "曾經熱賣" in is_repurchase:
                    prompt += "\n10. 【特別要求：老客回購】：這款商品曾大熱賣，請加入「之前買過超好用」、「這次要多囤幾組」等熱情回購語氣。"
                else:
                    prompt += "\n10. 【特別要求：初次上架】：請加入「想試試看」、「感覺很厲害」、「卡位等介紹」、「敲碗啦」等期待嘗鮮語氣。嚴禁使用「坐等開講」、「坐等講評」等死板詞彙！"
                
                inputs = [prompt]
                if uploaded_file is not None:
                    image_data = Image.open(uploaded_file)
                    inputs.append(image_data)
                    prompt += "\n11. 【圖片輔助】：觀察圖片中的特徵、顏色，把細節融入留言中。"
