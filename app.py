    ...
    if not product_name and uploaded_file is None:
        ...
        with st.spinner("AI 靈感湧現中，請稍候... (若數量較多請耐心等待)"):
            try:
                ...
                4. 禁忌命令與代稱 (極重要)：絕對禁止生成只有「+1」、「好用」、「買了」等過於零碎、沒頭沒尾的單詞！每一則留言都必須是【語意完整的自然句子】。同時，盡量搭配使用「這個」、「這罐」、「這組」、「這件」來代稱商品，讓語氣更自然。
                ...
                if "曾經熱賣" in is_repurchase:
                    ...
                    # 拔除「坐等」，換成更道地的台灣直播用語
                    prompt += "\n10. 【特別要求：初次上架】：這是新品，請在部分留言加入「想試試看」、「感覺很厲害」、「卡位等介紹」、「敲碗啦」、「迫不及待」等期待嘗鮮語氣。**嚴禁使用「坐等開講」、「坐等講評」等不自然的死板詞彙，請改用「卡位」、「敲碗」或「快介紹」！**"
