    ...
    if not product_name and uploaded_file is None:
        ...
        with st.spinner("AI 靈感湧現中，請稍候... (若數量較多請耐心等待)"):
            try:
                ...
                6. 留言者人設 (極重要)：請將這 {num_comments} 則留言，分配給以下這些不同身分的買家來發言：【{persona_instruction}】。請生動地模仿這些族群各自的口吻與關注點（例如：貴婦在意質感與包色、媽媽在意CP值與家人、長輩語速較慢在意健康實用、年輕人用語活潑等），讓留言區看起來是形形色色的人在互動。
...
if not st.session_state.history:
    st.sidebar.info("目前還沒有紀錄，設定好參數後點擊主畫面的「一鍵生成留言」吧！")
else:
    for i, record in enumerate(st.session_state.history):
        with st.sidebar.expander(f"📌 {record['product']} (最新)" if i == 0 else f"📂 {record['product']}", expanded=(i == 0)):
